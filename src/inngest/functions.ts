...
export const codeAgentFunction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    const sandboxId = await step.run('get-sandbox-id', async () => {
      const sandbox = await Sandbox.create('ai-website-builder-yash-test-3');
    ...
    const codeAgent = createAgent<AgentState>({
      ...
      lifecycle: {
        onResponse: async ({ result, network }) => {
          const lastAssitantTextMessage =
            lastAssitantTextMessageContent(result);

          if (lastAssitantTextMessage && network) {
            if (lastAssitantTextMessage.includes('<task_summary>')) {
              network.state.data.summary = lastAssitantTextMessage;
            }
          }
    ...
    const responseGenrator = createAgent({
      ...
      description: 'A respone generator',
